...
[ "$rc" = 0 ] && ok "three known kinds, no pending -> exit 0" || bad "exit $rc on a clean dir"
[ "$out" = "$(printf 'a|masked|EVAL\nb|skip|SKIP\nc|sha1|N/A')" ] && ok "one line per expectation, kind and disposition named" || bad "output: $(printf '%s' "$out" | tr '\n' ';')"
printf '%s' "$out" | grep -q notareplay && bad "a file whose stem is not a replay was listed" || ok "a non-replay stem is ignored"

echo "pending prose" > "$W/exp/d.pending"
